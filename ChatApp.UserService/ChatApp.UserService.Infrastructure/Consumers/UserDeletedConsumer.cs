                    ...
                    try
                    {
                        ...
                        consumer.Received += async (model, ea) =>
                        {
                            ...
                            try
                            {
                                ...
                                if (@event == null)
                                {
                                    throw new Exception("Invalid UserDeleted event received. Event is null.");
                                }

                                await _profileService.DeactivateUserAsync(@event);
                                channel.BasicAck(ea.DeliveryTag, false);
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, "Error processing UserDeleted event. Sending to DLQ.");
                                channel.BasicNack(ea.DeliveryTag, false, false);
                            }
                        };

                        channel.BasicConsume(QueueNames.UserDeletedQueue, false, consumer);

                        _logger.LogInformation("Consumer is now actively listening on {QueueName}", QueueNames.UserDeletedQueue);
